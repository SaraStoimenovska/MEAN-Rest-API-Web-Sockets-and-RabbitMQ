...
(function () {
  ...
  t = function () {
    ...
    r.prototype._connect = function () {
      var r, s, o, c;
      c = this.headers;
      o = this.errorCallback;
      s = this.closeEventCallback;
      if (!this._active) {
        ...
        return function (e) {
          ...
          while (r < s) {
            ...
            switch (t >> 4) {
              ...
              case 13:
                n = e[r++];
                o += String.fromCharCode((t & 31) << 6 | n & 63);
                break;
              case 14:
                n = e[r++];
                i = e[r++];
                o += String.fromCharCode((t & 15) << 12 | (n & 63) << 6 | (i & 63) << 0)
            }
          }
          return o
        }
      ...
      this.ws.onmessage = function (s) {
        return function (c) {
          var a, u, f, l, h, p, d, g, b, m, v;
          f = typeof ArrayBuffer !== "undefined" && c.data instanceof ArrayBuffer ? (a = new Uint8Array(c.data), typeof s.debug === "function" ? s.debug("--- got data length: " + a.length) : void 0, r(a)) : c.data;
          s.serverActivity = t();
          if (f === e.LF) {
          ...
          v = n.unmarshall(s.partialData + f, s.escapeHeaderValues);
          s.partialData = v.partial;
          b = v.frames;
          for (h = 0, p = b.length; h < p; h++) {
            ...
            switch (l.command) {
              case "CONNECTED":
                if (typeof s.debug === "function") {
                  s.debug("connected to server " + l.headers.server)
                }
                s.connected = true;
                s.version = l.headers.version;
                if (s.version === i.VERSIONS.V1_2) {
                  s.escapeHeaderValues = true
                }
                if (!s._active) {
                  s.disconnect();
                  return
                }
                s._setupHeartbeat(l.headers);
                if (typeof s.connectCallback === "function") {
                  s.connectCallback(l)
                }
                break;
              case "MESSAGE":
                m = l.headers.subscription;
                g = s.subscriptions[m] || s.onreceive;
                if (g) {
                  u = s;
                  if (s.version === i.VERSIONS.V1_2) {
                    d = l.headers["ack"]
                  ...
                  if (typeof s.debug === "function") {
                    s.debug("Unhandled received MESSAGE: " + l)
                  }
                }
                break;
              case "RECEIPT":
                if (l.headers["receipt-id"] === s.closeReceipt) {
                  s.ws.onclose = null;
                  s.ws.close();
                  s._cleanUp();
                  if (typeof s._disconnectCallback === "function") {
                    s._disconnectCallback()
                  }
                } else {
                  if (typeof s.onreceipt === "function") {
                    s.onreceipt(l)
                  }
                }
                break;
      ...
      return this.ws.onopen = function (e) {
        return function () {
          ...
          c["accept-version"] = i.VERSIONS.supportedVersions();
          c["heart-beat"] = [e.heartbeat.outgoing, e.heartbeat.incoming].join(",");
          return e._transmit("CONNECT", c)
        }
      }(this)
    };
